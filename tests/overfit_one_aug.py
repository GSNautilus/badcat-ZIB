"""Phase 2A diagnostic: do generations vary by reference image?

If yes -> the augmented adapter retained reference-dependent routing
            (architecture has reference-conditioning capacity even under
             constant-target training pressure)
If no  -> the adapter collapsed to a constant bias regardless of reference
            (constant-target training selected for content memorization)

Sample matrix: 3 references x 3 prompts = 9 images, plus baselines.
References:
  - starry: Starry Night (the trained reference)
  - cat:    The training content image used as reference (radically different)
  - red:    Solid red square (unseen distribution)
"""
...
SEED = 42
STEPS = 9
CKPT = os.path.join(os.path.dirname(__file__), "..", "checkpoints", "phase2a_overfit_aug.pt")
OUTDIR = os.path.join(os.path.dirname(__file__), "..", "outputs", "phase2a")
...
def main():
    ...
    for name, img in refs_pil.items():
        ...
        print(f"  ref={name}: SigLIP {h}x{w}")

    image_h_patched = HEIGHT // 16
    image_w_patched = WIDTH // 16

    # Baselines (no adapter, one per prompt)
    ...
    # With adapter, all combinations
    print("\nGenerating with adapter (3 refs x 3 prompts)...")
